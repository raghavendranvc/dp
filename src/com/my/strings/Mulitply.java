...
public class Mulitply {
	...
	/**
	 * 2468 * 141 ========== 2 4 6 8 9 8 7 2 0 2 4 6 8 0 0
	 *
	 * @param A
	 * @param B
	 * @return
	...
	public String multiply(String A, String B) {
		...
		for (int i = 0; i < b.length; i++) {
			...
			for (int j = 0; j < a.length; j++) {
				value = (long) (value
						+ ((a[a.length - 1 - j] - '0') * (b[b.length - 1 - i] - '0')) * (Math.pow(10, j)));
				System.out
						.println("computing at i=" + value + " a=" + a[a.length - 1 - j] + " b=" + b[b.length - 1 - i]);
			}
	...
	//TODO read this.
	//Read how we store the multiplication in char array
	//1.Reverse the array. Create new array of length m+n
	//2. For each char, do multiple, store in i+j along with value at (i+j)+carry+d1*d2
	//3. Handle carry for every digit multiplication.
	...
	public String multiplyCopied(String num1, String num2) {
		...
		while (ret.charAt(pos) == '0' && pos < ret.length())
			pos++;
		return ret.substring(pos);
	}

	/*
	 * 2 4 6 8 X 4 32 240 1600 8000
	 * 
	 * 
	 */

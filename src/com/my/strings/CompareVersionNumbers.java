...
public class CompareVersionNumbers {
    ...
    public int compareVersion(String A, String B) {
        ...
        for(;i<versionsA.length && i < versionsB.length;i++){
            /*long va = Long.valueOf(versionsA[i]) ;
            long vb = Long.valueOf(versionsB[i]) ;
            System.out.println("i="+i+" va="+va+" vb="+vb);
            if(va>vb){
                return 1;
            } else if (va < vb) {
                return -1;
        ...
        System.out.println("i="+i+" versionsA.length="+versionsA.length+" versionsB.length="+versionsB.length);

...
public class ThereeColorSort {
	...
	public void sortColors(int[] nums) {
		...
		int firstPotentialOneIndex = 0;
		int firstPotentialTwoIndex = nums.length - 1;

		int i = 0;
		while (i <= firstPotentialTwoIndex) {
			...
			if (nums[i] == 0) {
				swap(nums, i, firstPotentialOneIndex);
				firstPotentialOneIndex++;
				i++;
				continue;
			}

			if (nums[i] == 2) {
				swap(nums, i, firstPotentialTwoIndex);
				firstPotentialTwoIndex--;
				continue;
			}
		}

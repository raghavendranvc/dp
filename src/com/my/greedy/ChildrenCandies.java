...
public class ChildrenCandies {
     ...
     *
     *
     * When you move, if increment happens, then currentCandies++
     * if decreement happens, then currentCandies--;
     *
     * In the worst case, where all students are in reverse order (means highest ranking student first)
     * then we need make sure that the first candidate gets, the size of the array(total number of students)
     * similarly best case, is where all students are orders in their rank, then we simply has to increment
     *
     * At each position, we will calculate the minimum of Chocs received by any student.
     * If say x, then we make x to 1 by substracting all by x-1
     *
    ...
    public int candyCopied(ArrayList<Integer> A) {
        ...
        /*
            ...
            1. Make sure that the high ranking right side guy gets more than his left side.
            2. Just give one extra compared to the left guy.
         */
        for(int i=0;i<n-1;i++){
        ...
        /*
            ...
            So we should fix it by reversing from the rear end

            Next give candiies by comparing 2 students starting from the rear end

            1. Make sure that the high ranking left side guy gets more than his right side.

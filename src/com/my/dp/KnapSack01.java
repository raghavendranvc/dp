...
public class KnapSack01 {
	...
	public int maxValue(int[] values,int[] weights,int capacity,int selectedWeight,int currentValue){
		...
		/*
		 * If - When the weight is selection
		 * Else - When the weight is not selected
		 */
		...
		if((capacity - weights[selectedWeight]) >= 0){
			 ...
			 */
			int value = maxValue(values,weights,capacity - weights[selectedWeight],selectedWeight+1,currentValue+values[selectedWeight]);
	...
	private int SimpleKnapSackRecus(int[] values,int[] weights,int capacity,int i){
		...
		if(weights[i-1] > capacity){
			 ...
			 */
			return Math.max(
					values[i-1]+ SimpleKnapSackRecus(values,weights,capacity-weights[i-1],i-1),
	...
	private int SimpleKnapSackReverseRecus(int[] values,int[] weights,int capacity,int selectedWeight){
		...
		if(weights[selectedWeight] > capacity){
			 ...
			 */
			return Math.max(
					values[selectedWeight]+ SimpleKnapSackReverseRecus(values,weights,capacity-weights[selectedWeight],selectedWeight+1),
		...
		for(int i=0;i<=weights.length;i++){
			for(int currentWeight=0;currentWeight<=capacity;currentWeight++){
				if(i==0 || currentWeight==0){
					...
					table[i][currentWeight] = Math.max(values[i-1]+table[i-1][currentWeight-weights[i-1]], table[i-1][currentWeight]);
				}
			}

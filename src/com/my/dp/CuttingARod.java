...
public class CuttingARod {
	...
	public int maxValueByCutting(int[] lengths,int[] values,int start,int end){
		...
		/* 
		 * Case 2: Cut it at position k (start<k<end). 
		 * Have the max of both the positions 
		 * 
		 * 
		 */
...
//			currentMax = Math.max(currentMax,maxValueByCutting(lengths, values,start,k)+maxValueByCutting(lengths, values,k,end));
	...
	public int maxValueSimpleRecu(int[] values,int size){
		...
		/*
		 * i=0 means size = 1
		 * so it moves from 0 to size-1
		 * when i=size-1 then no cut is made. 
		 */
	...
	public int maxValueIter(int[] values,int size){
		...
		/*
		 * From each i from 1 to size (for all sizes)
		 * For rod size 1 : we calculate what is the max we get by cutting it
		 * For rod size 2 : we calculate what is the max we get by cutting it
		 */
		for(int i=1;i<=size;i++){
			...
			for(int j=0;j<i;j++){ 
				...
				System.out.println("j="+j+" value="+values[j]+" tables["+(i-j-1)+"]="+tables[i-j-1]+ " valGot="+valGot);

...
public class LargestIndependentSubsetOfBST {
	...
	public int getLargestISubSetOfBST(Node root) {
		...
		int rootSubset = 1
				+ ((root.left == null) ? 0
						: getLargestISubSetOfBST(root.left.left) + getLargestISubSetOfBST(root.left.right))
				+ ((root.right == null) ? 0
						: getLargestISubSetOfBST(root.right.left) + getLargestISubSetOfBST(root.right.right));
	...
	public int getLargestISubSetOfBST(LisNode root) {
		...
		/*
		 * if(root.left == null && root.right == null){ root.lisVal = 1; return
		 * root.lisVal; }
		 */

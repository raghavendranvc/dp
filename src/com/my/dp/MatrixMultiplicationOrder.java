...
public class MatrixMultiplicationOrder {
	...
	/*
	 * Eg : (order,1,4) for order (10,30,20,20,40)
	 * k=1 to 3 // closed parenthesis movies from position 1 to 4
	 * 
	 */
	...
	public int getMinMultiplications(int[] order,int i,int j){
		...
		for(int k=i;k<j;k++){
			...
			int count = getMinMultiplications(order,i,k) + (order[i-1]*order[k]*order[j]) +  getMinMultiplications(order,k+1,j);
...
//			for(int i=1,j=chainLength;i<=(n-chainLength);i++,j++){
			...
			for(int i=1,j=chainLength;j<n;i++,j++){	
				...
				for(int k=i;k<j;k++){
					...
					System.out.print("\t\tk="+k +" ["+i+"]["+k+"]="+table[i][k]+"   "+p[i-1]+"*"+p[k]+"*"+p[j]+"    ["+(k+1)+"]["+j+"]="+table[k+1][j]);
	 ...
	 * 
	 * (AB)C or A(BC) = 2 ways
	 * (AB)CD or A(BC)D or AB(CD) = 3 ways
	 * (AB)CDE or A(BC)DE or AB(CD)E or ABC(DE) = 4 ways    
	 */
	...
	public int getMinMultiplications1(int[] order,int i,int j){
		...
		*/
		for(int k=i;k<=j;k++){
			int count = getMinMultiplications(order,i,k-2) + (order[k-1]*order[k]) +  getMinMultiplications(order,k+1,j);
	...
	public static void main(String[] args) {
		...
		// 1 implies position of closed parenthesis for n = 4 then (0,1) (1,2) (1,3) (1,4)

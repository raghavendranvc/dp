...
public class LongestIncreasingSubsequnce {
	...
	public int longestIncreasingSubsequnce(int[] a,int seqLength,int[] lisList){
		...
		for(int i=1;i<seqLength;i++){
			...
			System.out.print("LCS Array=");printArray(newLisList);
			if(a[i-1] < lastNum && max < (subMax+1)){
				max = subMax+1;
				lisList = newLisList;
				System.out.print("New LCS Array=");printArray(newLisList);
			}
		}
	...
	public int lisIterative(int[] a,int seqLength){
		...
		for(int i=1;i<seqLength;i++){   //For each sequence length from 1 to seqLength-1 
			for(int j=0;j<i;j++){
				...
				if(a[j]<a[i]){  // a[1],a[2],a[3] ..... a[n-1]  for each check if all elements before it are less than it.
					lcsLengths[i] = Math.max(lcsLengths[i], lcsLengths[j]+1);
				}
				System.out.println();
			}
			System.out.print("LCS Array=");printArray(lcsLengths);
		}
	...
	public int lisListIterative(int[] a,int seqLength){
		...
		for(int i=1;i<seqLength;i++){  // a[1],a[2],a[3] ..... a[n-1]  for each check if all elements before it are less than it. 
			for(int j=0;j<i;j++){
				System.out.print("Check::a["+i+"]="+a[i]+", a["+j+"]="+a[j]);
				if(a[j]<a[i]){
		...
		System.out.print("LCS Array=");printArray(a);
		printList(lcsLists);
		return max;
	}
	...
	private int lcsNlogNAlgo(int [] a){
		...
		for(int i=1;i<totalSize;i++){
			...
			if(a[i] < listOfTailElements[0] ){ // New element is less than the base list of length=1
				...
				/*
				 * New Element is greater than all elements. (We clone the last list and add the new element to that cloned list) 
				 */
				len++;

...
public class MinimumJumps {
	 ...
	 public int jumpCopied(ArrayList<Integer> A) {
	        ...
	        // maxReach simply checks the maximum distance that can be moved using the values in the array. 
	        // It can reach beyond any point.
	        
	        // we start with the second one
	        for(int i=1; i<A.size(); i++){
	        	
	            if(maxReach < i){  //At any point of time, our maxReach is less than the current position, then there is no path
	                return -1;
	            }
	            ...
	            if(lastReach < i){
	                jumps++; 
	                //We jump only when necessary. So when our first/initial jump works, we will remain there
